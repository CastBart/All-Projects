	...
	, m_B_Rect(bRect)
{
	...
	m_A_ButtonTexture = std::make_shared<sf::Texture>();
	m_B_ButtonTexture = std::make_shared<sf::Texture>();
	...
	// loading the textures for a and b buttons
	if (!m_A_ButtonTexture->loadFromFile("./resources/textures/a-button.png"))
	{
		std::cout << "A texture didn't load" << std::endl;
	}
	if (!m_B_ButtonTexture->loadFromFile("./resources/textures/b-button.png"))
	{
		std::cout << "B texture didn't load" << std::endl;
	}
	...
	// set up the rectangles for a button
	m_A_Rect->setTexture(&(*m_A_ButtonTexture), true);
	m_A_Rect->setSize(sf::Vector2f(m_A_ButtonTexture->getSize()));
	m_A_Rect->setPosition(sf::Vector2f(800 - 220, 500));

	// set up the rectangles for b button
	m_B_Rect->setTexture(&(*m_B_ButtonTexture), true);
	m_B_Rect->setSize(sf::Vector2f(m_B_ButtonTexture->getSize()));
	m_B_Rect->setPosition(sf::Vector2f(800 - 220, 550));
...
void MenuSystem::receive(const EvAssetsLoaded& e)
{
	...
	case GameState::MainMenu:
		...
		m_gui->addLabel("Select", 20, sf::Vector2f(800 - 100, 520), font[Assets::Fonts::GUI::Content]);
		m_gui->addButton(
	...
	case GameState::OptionsScreen:
		...
		m_bgSprite->setTexture(guiTexture[Assets::Textures::GUI::Background]);
		m_gui->addLabel("Select", 20, sf::Vector2f(800 - 100, 520), font[Assets::Fonts::GUI::Content]);
		m_gui->addLabel("Back", 20, sf::Vector2f(800 - 100, 570), font[Assets::Fonts::GUI::Content]);
		m_gui->addSlider(
			font[Assets::Fonts::GUI::Content]
			, "Music Volume"
			, 25u
	...
	case GameState::Upgrade:
	{
		...
		const auto& wheelCost = m_cost * (wheels + 1);
		const auto& engineCost = m_cost * (engine + 1);
		const auto& handlingCost = m_cost * (handling + 1);

		m_gui->addLabel("MONEY : " + std::to_string(currency), 20.0f, sf::Vector2f(600.0f, 20.0f), font[Assets::Fonts::GUI::Content], sf::Color::Red);
		m_gui->addLabel("COST : " + std::to_string(wheelCost * (wheels + 1)), 20.0f, sf::Vector2f(500.0f, 90.0f), font[Assets::Fonts::GUI::Content], sf::Color::Blue);
		m_gui->addLabel("COST : " + std::to_string(engineCost * (engine + 1)), 20.0f, sf::Vector2f(500.0f, 500.0f), font[Assets::Fonts::GUI::Content], sf::Color::Blue);
		m_gui->addLabel("COST : " + std::to_string(handlingCost * (handling + 1)), 20.0f, sf::Vector2f(500.0f, 300.0f), font[Assets::Fonts::GUI::Content], sf::Color::Blue);

		m_gui->addLabel("Buy", 20, sf::Vector2f(800 - 100, 520), font[Assets::Fonts::GUI::Content]);
		m_gui->addLabel("Back", 20, sf::Vector2f(800 - 100, 570), font[Assets::Fonts::GUI::Content]);
	...
	, double dt)
{
	...
	if (m_gameState == GameState::TrackSelection || m_gameState == GameState::CarSelection || m_gameState == GameState::OptionsScreen || m_gameState == GameState::Upgrade)
	{
		//updateRect();
		m_A_Rect->setScale(sf::Vector2f(1, 1));
		m_B_Rect->setScale(sf::Vector2f(1, 1));
	}
	else if (m_gameState == GameState::MainMenu)
	{
		m_A_Rect->setScale(sf::Vector2f(1, 1));
		m_B_Rect->setScale(sf::Vector2f(0, 0));
	}
	else
	{
		m_A_Rect->setScale(sf::Vector2f(0, 0));
		m_B_Rect->setScale(sf::Vector2f(0, 0));
	}
...
void MenuSystem::upgradeWheelsBtn()
{
	...
	if (wheels < 3)
	{
		if (currency > cost)
		{
			...
			currency -= cost;
			wheels++;
			m_eventManager.emit<EvUpgradeWheel>(wheels, cost);
			m_eventManager.emit<EvChangeGameState>(GameState::Upgrade);
		}
		else
		{
...
void MenuSystem::upgradeHandlingBtn()
{
	...
	if (handling < 3)
	{
		if (currency > cost)
		{
			...
			currency -= cost;
			handling++;
			m_eventManager.emit<EvUpgradeHandling>(handling, cost);
			m_eventManager.emit<EvChangeGameState>(GameState::Upgrade);
		}
		else
		{
...
void MenuSystem::upgradeEngineBtn()
{
	...
	if (engine < 3)
	{
		if (currency > cost)
		{
			...
			currency -= cost;
			engine++;
			m_eventManager.emit<EvUpgradeEngine>(engine, cost);
			m_eventManager.emit<EvChangeGameState>(GameState::Upgrade);
		}
		else
		{
