...
void MotionSystem::updateCarMovement(entityx::EntityManager & entities, entityx::EventManager & events, double dt)
{
	...
	float minSteerForce = 0.5f;
	//the maximum degree of rotation
	float maxSteerForce = 0.9f;
	//breaking value
	float breakingVal = 5.0f;
	//delta time
	...
	for (auto& carId : m_carIds)
	{
		...
		{
			//assign player handle
			player = entity.component<Player>();

			///////////////////UPGRADE SCREEN///////////////////// (rushed 1h till due hour)
			if (player->m_engine == 1)
			{
				car->m_maxAccel = 1000.0f;
			}
			else if (player->m_engine == 2)
			{
				car->m_maxAccel = 1500.0f;
			}
			else if (player->m_engine == 3)
			{
				car->m_maxAccel = 2000.0f;
			}
			if (player->m_handling == 1)
			{
				minSteerForce = 0.2f;
				maxSteerForce = 0.4f;
			}
			else if (player->m_handling == 2)
			{
				minSteerForce = 0.4f;
				maxSteerForce = 0.7f;
			}
			else if (player->m_handling == 3)
			{
				minSteerForce = 0.7f;
				maxSteerForce = 1.1f;
			}
			...
			else if (player->m_wheel == 2)
			{
				breakingVal = 5.0f;
			}
			else if (player->m_wheel == 3)
			{
				breakingVal = 10.0f;
			}
		}
