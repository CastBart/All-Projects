...
void Game::createSystems()
{
	...
	std::shared_ptr<sf::RectangleShape> aRect = std::make_shared<sf::RectangleShape>();
	std::shared_ptr<sf::RectangleShape> bRect = std::make_shared<sf::RectangleShape>();

	m_systemManager.add<LevelSystem>(m_entityManager, m_eventManager, resourceHolder);
	m_systemManager.add<MenuSystem>(m_eventManager, m_entityManager, m_window, gui, controller, resourceHolder, textRects, bgSprite , lbRect , rbRect, aRect, bRect);
	m_systemManager.add<MenuRenderSystem>(gui, m_window, textRects, bgSprite, lbRect, rbRect, aRect, bRect);
	...
	m_systemManager.add<HudSystem>(m_window, m_entityManager, m_eventManager, resourceHolder->m_fonts, view);
