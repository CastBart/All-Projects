...
/// 
/// </summary>
HudSystem::HudSystem(sf::RenderWindow & window, entityx::EntityManager& entityManager, entityx::EventManager& newEvent, std::shared_ptr<FontBank> font, std::shared_ptr<sf::View> view)
	: m_window(window)
			...
			if (otherCarId != carId)
			{
				...
				if (carEntity.has_component<Player>())
				{
					if (carLap == otherCarLap)
					{
						...
						{
							if (m_playerRacePos < m_aiId.size() + 1)
							{
								m_playerRacePos++;
							}
						}
...
void HudSystem::receive(const EvLapCompleted& e)
{
	...
	if (e.m_carId == m_playerId) // check if its the player car
	{
		...
		if (m_lapUndone == 0) // if the player didnt go backwards
		{
			...
			if (carComp->m_lap >= 1)
			{
				//carComp->m_currency += 1000;
			//	m_event.emit<EvChangeGameState>(GameState::TrackSelection);
			}
		}
		else
		{
